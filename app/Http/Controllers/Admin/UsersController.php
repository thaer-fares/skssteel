...
class UsersController extends AdminController
{
    ...
    {
        $res =Http::timeout(120)->accept('application/json')
        ->post( 'http://transport.test/api/login', [
    ...
    $res2 =Http::timeout(120)->accept('application/json')
     ->get( 'http://transport.test/api/driverInfo/2?token='.$res_json->authorisation->token);


    self::$data['res_json2'] =json_decode($res2->body());





// $res2 =Http::timeout(120)->accept('application/json') ->withHeaders([

//     'api-key' => 'tAtyqxMyx2minOanLIJjIwwh7tN9dHSKTr0BwvetKuL0lIP9NK',

//     'Authorization' => 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwOi8vdHJhbnNwb3J0LnRlc3QvYXBpL2xvZ2luIiwiaWF0IjoxNjcyMjI1ODUwLCJleHAiOjE2NzIyMjk0NTAsIm5iZiI6MTY3MjIyNTg1MCwianRpIjoiSlBwWGlpVTVJVTdGY1FQeSIsInN1YiI6IjEiLCJwcnYiOiIyM2JkNWM4OTQ5ZjYwMGFkYjM5ZTcwMWM0MDA4NzJkYjdhNTk3NmY3In0.-dAeE6rZ9HhneE1UN0IMQKh8LuT51SyNgVA0QJbyH7A',


// ])
//  ->get( 'app.palwakf.ps/eaffair/public/api/v1/amlak/employeesByID?term=801939547');
//  ->get( 'http://transport.test/api/driverInfo/2');
//  $res3=json_decode($res2->body());



// self::$data['res_json2'] =json_decode($res2->body());

    // print_r( $data['res_json2']->info->id);
    // die();
    // dd( $data['res_json2']->info);
    ...
    {
        return view('admin.users.driver', self::$data);
}

public function getDriverByIdentity(Request $request)
{

    $driverIdentity = $request->get('identity');
    $res =Http::timeout(120)->accept('application/json')
    ->post( 'http://transport.test/api/login', [
...
$res_json = json_decode($res->body());
$res2 =Http::timeout(120)->accept('application/json')
 ->get( 'http://transport.test/api/driverInfo/' . $driverIdentity . '?token='.$res_json->authorisation->token);


self::$data['res_json2'] =json_decode($res2->body());


    return view('admin.users.driver_info', self::$data);
}
